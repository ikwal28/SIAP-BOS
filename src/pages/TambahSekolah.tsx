...
export default function TambahSekolah() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}?action=tambahSekolah`, {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        ...
        toast.error(data.message || 'Terjadi kesalahan saat menambahkan sekolah');
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Tambah Sekolah</h2>
          <p className="text-gray-500 text-sm">Tambahkan data sekolah penerima BOS baru beserta akun adminnya.</p>
        </div>
      </div>
      ...
      >
        <div className="p-6 space-y-8">
          ...
          {/* Bagian Akun */}
          <section>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4 border-b border-gray-100 dark:border-gray-700 pb-2">Informasi Akun Admin Sekolah</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-600 dark:text-gray-400">Username Login</label>
                <input required type="text" name="username" value={formData.username} onChange={handleChange} className="w-full p-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 text-gray-800 dark:text-white outline-none transition" placeholder="Buat username unik" />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-600 dark:text-gray-400">Password Login</label>
                <input required type="password" name="password" value={formData.password} onChange={handleChange} className="w-full p-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 text-gray-800 dark:text-white outline-none transition" placeholder="Buat password yang kuat" />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-600 dark:text-gray-400">Tipe Akses</label>
                <select name="tipeAkses" value={formData.tipeAkses} onChange={(e:any) => handleChange(e)} className="w-full p-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 text-gray-800 dark:text-white outline-none transition cursor-pointer form-select appearance-none">
              ...
              {formData.tipeAkses === 'rentang_waktu' && (
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-600 dark:text-gray-400">Batas Waktu Akses</label>
                  <input required type="date" name="batasWaktu" value={formData.batasWaktu} onChange={handleChange} className="w-full p-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 text-gray-800 dark:text-white outline-none transition" />
                </div>
              )}
            </div>
          </section>
        ...
        <div className="p-6 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-700 flex justify-end gap-4">
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white px-8 py-2.5 rounded-xl flex items-center gap-2 font-semibold shadow-lg shadow-indigo-500/30 transition disabled:opacity-70"
          >
      ...
      <LoadingOverlay isLoading={loading} message="Menambahkan sekolah baru..." />
    </div>
  );
}
