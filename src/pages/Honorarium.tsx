...
export default function Honorarium() {
  ...
  useEffect(() => {
    if (printId && honors.length > 0) {
      const match = honors.find(h => h.id === printId);
      if (match) {
        setPrintItem(match);
        if (autoPrint) {
          setTimeout(() => {
  ...
  return (
    <div className="space-y-6">
      {autoPrint && (
        <div className="print:hidden bg-indigo-50/90 dark:bg-gray-900 border border-indigo-100 dark:border-gray-800 rounded-3xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 max-w-6xl mx-auto mb-6 backdrop-blur-md shadow-md animate-fade-in font-sans">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-indigo-600 text-white rounded-xl">
              <Printer size={18} />
            </div>
            <div>
              <h3 className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight font-sans">Pratinjau Cetak Honorarium PDF</h3>
