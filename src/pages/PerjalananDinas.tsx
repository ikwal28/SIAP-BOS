...
export default function PerjalananDinas() {
  ...
  useEffect(() => {
    if (printId && spds.length > 0) {
      const match = spds.find(s => s.id === printId);
      if (match) {
        setPrintItem(match);
        if (autoPrint) {
          setTimeout(() => {
      ...
      {autoPrint && (
        <div className="print:hidden bg-indigo-50/90 dark:bg-gray-900 border border-indigo-100 dark:border-gray-800 rounded-3xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 max-w-6xl mx-auto mb-6 backdrop-blur-md shadow-md animate-fade-in">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight font-sans">Pratinjau Cetak Perjalanan Dinas (SPD) PDF</h3>
